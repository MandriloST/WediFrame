...
> **Zadnje ažurirano:** 2026-08-10 (v11)
...
- [~] Thumbnail generiranje za fotke (background job) — **kod isporučen 2026-08-10 (v11)**: `ThumbnailWorker` (BackgroundService, poll DB stanja Confirmed+Pending), libvips/NetVips generator (JPEG 640px, HEIC podržan), `IThumbnailGenerator`+`IObjectStorage` prošireni (Download/Upload), `MediaItem.ThumbnailStatus` (Pending/Ready/Failed), frontend HEIC lightbox fallback. **Traži novu migraciju `AddThumbnailStatus`** + NuGet restore (NetVips). Galerija automatski koristi `thumbnailUrl` kad job odradi
